...
from .masking import JEPACurriculumMasker


class JEPATrainer:
    """Training loop for MultiModalJEPA."""
    ...
    def __init__(
        ...
        if self.is_main:
            ...
            self.logger = Logger(output_dir, name='jepa_train', use_tensorboard=True, rank=self.rank)
        else:
            self.logger = None
        ...
        self.masker = JEPACurriculumMasker.from_config(self.config)
    ...
    def train_epoch(self, loader, epoch: int) -> float:
        ...
        tracker.add_meter('mask_ratio')

        for batch_idx, batch in enumerate(loader):
            batch_start = time.time()
            batch = move_to_device(batch, self.device)
            mask_ratio = self.masker.get_ratio(epoch)

            with autocast(device_type='cuda', enabled=self.use_amp):
                outputs = self.model(batch, mask_ratio=mask_ratio, return_loss=True)
                loss = outputs['loss']
            ...
            tracker.update(
                loss=loss.item(),
                mask_ratio=float(outputs['mask'].float().mean().item()),
